...
### How do the values of any relevant fields of the class change from this specific request? If no values got changed, explain why.
 When we `/add-message` with `s=message` and `user=username` parameters, it will append the username and message user inputed to the String `chat`. In this case, we have `s=How%20are%20you` and `user=yash`. The updated `chat` will now contain `"jpolitz: Hello \n yash: How are you \n"`.<br/><br/> 
